...
async def generar_reporte_excel_supabase(db: psycopg2.extensions.connection = Depends(get_db)):
    """
    Genera un reporte Excel y lo sube a Supabase Storage para su descarga.
    """
    try:
        logger.info("Iniciando generación de reporte Excel para Supabase Storage")
        
        # Primero generamos el Excel como en el método normal
        cursor = db.cursor()
        logger.debug("Consultando datos de empleados y sucursales")
        cursor.execute("""
            SELECT e.id, e.nombre, e.talla, s.nombre as sucursal, s.manager, s.zona, s.region
            FROM empleados e
            JOIN sucursales s ON e.sucursal_id = s.id
        ...
        datos = cursor.fetchall()
        logger.debug(f"Obtenidos {len(datos)} registros de empleados")
        
        df = pd.DataFrame(datos)
        
        logger.debug("Consultando resumen por talla")
        cursor.execute("""
            SELECT talla, COUNT(*) as cantidad
            FROM empleados
            GROUP BY talla
        ...
        logger.debug("Consultando resumen por sucursal")
        cursor.execute("""
            SELECT 
                s.nombre as sucursal,
                COUNT(CASE WHEN e.talla = 'XS' THEN 1 END) as XS,
        ...
        logger.debug(f"Generando archivo Excel: {nombre_archivo}")
        # Crear archivo Excel con múltiples hojas
        with pd.ExcelWriter(ruta_archivo) as writer:
            df.to_excel(writer, sheet_name='Detalle', index=False)
            df_resumen.to_excel(writer, sheet_name='Resumen', index=False)
        ...
        logger.debug("Archivo Excel generado correctamente")
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s)
                RETURNING id
            """, (nombre_archivo, datetime.now(), 'excel_supabase', url_descarga, expiracion))
            
            reporte_id = cursor.fetchone()['id']
            db.commit()
            
            logger.info(f"Reporte registrado con ID: {reporte_id}")
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s)
                RETURNING id
            """, (nombre_archivo, datetime.now(), 'excel_local'))
        ...
        logger.error(f"Error al generar reporte Excel para Supabase: {str(e)}")
        logger.exception("Traceback completo:")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error al generar reporte: {str(e)}"
